...
spec = importlib.util.spec_from_file_location(
    "tfidf_name",
    "code/business_entity_resolution/src/blocking/tfidf_name.py"
)
...
print("=== Testing Route 2 on 1K Samples ===")
print(f"S1: {s1.shape}, S2: {s2.shape}, S3: {s3.shape}")

res1 = mod.retrieve_tfidf_name(s1, s2, s3, top_k=20, chunk_size=250)
...
# Test Determinism
res2 = mod.retrieve_tfidf_name(s1, s2, s3, top_k=20, chunk_size=250)
pd.testing.assert_frame_equal(res1, res2)
print("Determinism test: PASSED (consecutive runs produce identical results)")

# Check candidate schema & types
...
assert (res1["route"] == "tfidf_name").all(), "Invalid route tag"
